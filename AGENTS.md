...
| 文件 | 用途 |
|---|---|
| `question_schema.json` | Step 2 使用的 Structured Outputs Schema（题目结构） |
| `answer_schema.json` | Step 4 使用的 Structured Outputs Schema（答案与解析） |
| `prompt_step2.md` | Step 2 的 System Prompt 与 User Prompt 模板 |
| `prompt_step4.md` | Step 4 的 System Prompt 与 User Prompt 模板 |
...
此步骤为可复用模块，在 Step 2 之后和 Step 4 之后各调用一次，通过参数控制校验范围。
...
**输出：** 符合 `answer_schema.json` 的答案与解析 JSON

**规则：**

1. 调用 OpenRouter API，使用 Structured Outputs 功能，逐题推算正确答案并生成完整解析。
2. Schema 使用 `answer_schema.json`，设置 `strict: true`。
3. API 配置同 Step 2（读取根目录 `.env` 文件）。
4. Prompt 参见 `prompt_step4.md`。
5. **以顶层题目为单位调用：** 每道顶层题目单独发起一次 API 请求。父题的 `stem` 作为所有子题的上下文，整棵子题树在同一次请求中处理。
6. **答案 Schema 为镜像树结构：** `answer_schema.json` 的 `sub_answers` 数组与 `question_schema.json` 的 `sub_questions` 按位置一一对应。
